...
module.exports = function(session) {
	...
	oracleDbStore.prototype.createDatabaseTable = function(cb) {
		...
		var sql = `CREATE TABLE ${this.options.schema.tableName} (
  ...
  PRIMARY KEY (${this.options.schema.columnNames.session_id})
	...
	oracleDbStore.prototype.get = function(session_id, cb) {
		...
		t this.connection.execute(sql, params, { outFormat: oracledb.ARRAY, fetchInfo: { "DATA": { type: oracledb.STRING} } }, // Fetch as a String instead of a Stream
	...
	oracleDbStore.prototype.set = function(session_id, data, cb) {
		...
		var sql = `MERGE INTO ${this.options.schema.tableName} trg
...
VALUES (:sessionid,:expires,:attributes)`
	...
	oracleDbStore.prototype.touch = function(session_id, data, cb) {
		...
		var sql = `UPDATE ${this.options.schema.tableName} SET ${this.options.schema.columnNames.expires} = :expires WHERE ${this.options.schema.columnNames.session_id} = :sessionid`;
		var params = {
			expires: expires,
			sessionid: session_id
		};
	...
	oracleDbStore.prototype.length = function(cb) {
		...
		this.connection.execute(sql, function(error, result) {
			...
			debug_log('Number of sessions result: ' +result.rows[0]);
			var count = result.rows[0] ? result.rows[0][0] : 0;
	...
	oracleDbStore.prototype.clearExpiredSessions = function(cb) {
		...
		var sql = `DELETE FROM ${this.options.schema.tableName} WHERE ${this.options.schema.columnNames.expires} < :expir`;
